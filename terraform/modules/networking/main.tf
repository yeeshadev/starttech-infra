...
resource "aws_security_group" "backend" {
  ...
  description = "Allow traffic from ALB to backend on port 8080"
  vpc_id      = aws_vpc.main.id
  ...
  # SSH restricted to admin IP in production — here open for dev; restrict in prod
  ingress {
    description = "SSH"
    from_port   = 22
    to_port     = 22
  ...
  tags = merge(local.tags, { Name = "${var.project_name}-${var.environment}-backend-sg" })
}
...
resource "aws_security_group" "redis" {
  ...
  tags = merge(local.tags, { Name = "${var.project_name}-${var.environment}-redis-sg" })
}
